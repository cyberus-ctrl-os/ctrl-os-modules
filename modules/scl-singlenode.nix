...
in
{
  options.services.scl-singlenode = {
    ...
    computeApi = {
      ...
      maxCpus = mkOption {
        description = ''
          The maximum number of cores that all vms, specifyed by
          `virtualisation.vms.virtualMachines` are allowed to use.
          The underlying tool assings static portions of

          By default, the maximum number of CPUs is determined at runtime using
          the host cpu core count via nproc.
          An overcommitment of CPU cores is possible.
        '';
      ...
      maxMemory = mkOption {
        description = ''
          The maximum amount of memory in megabytes that the virtual machines, specifyed by
          `virtualisation.vms.virtualMachines` are allowed to use.

          By default, 75 percent of the host memory, determined at runtime is made available to the virtual machines.

          An overcommitment of memory is possible, but not recommended.
        '';
        type = with types; nullOr ints.positive;
        default = null;
      };
