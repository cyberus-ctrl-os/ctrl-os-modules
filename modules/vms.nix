...
let
  ...
  # Virtual machine management related options and ports
  sclEnvironment = {
    SCLCTL_API_URL = config.programs.sclctl.url;
    SCLCTL_CA_CERT_FILE = config.programs.sclctl.mtls.caCertificate;
    SCLCTL_CLIENT_CERT_FILE = config.programs.sclctl.mtls.cert;
    SCLCTL_CLIENT_KEY_FILE = config.programs.sclctl.mtls.key;
  };
      ...
      options = {
        ...
        cores = mkOption {
          ...
          description = ''
            ...
            The number cannot be higher than `virtualisation.vms.maxCores`
          '';
        };
        ...
        memorySize = mkOption {
          ...
          description = ''
            ...
            The number must not be higher than `virtualisation.vms.maxMemory
          '';
        };
  ...
  mkVMService =
    ...
    let
      ...
      imageName = "${name}-${builtins.hashFile "sha1" options.image}";
    in
    {
      name = "vm-${name}";
      value = {
        ...
        '';
        script = ''
          if ! sclctl volume show ${serviceName} ${imageName}; then
            cp ${options.image} ${config.services.scl-singlenode.imageRegistry.dataDir}/${imageName}.img
            sclctl volume create --size ${toString options.imageSize} --url http://localhost:${toString config.services.scl-singlenode.imageRegistry.listenPort}/${imageName}.img ${serviceName} ${imageName}
            while [ "$(sclctl volume show ${serviceName} ${imageName} | jq -r ".status")" != "active" ]; do sleep 1; done
            rm ${config.services.scl-singlenode.imageRegistry.dataDir}/${imageName}.img
          fi
          while sclctl vm show ${serviceName} ${name}; do
            sclctl vm update ${serviceName} ${name} stopped  || true
            sclctl vm delete ${serviceName} ${name} || true
            sleep 1
          done

          sclctl vm create ${serviceName} ${name} \
            --vcpu ${toString (options.cores)} \
            --ram ${toString (options.memorySize)} \
            --boot-volume name=${imageName}  \
            --network-device-name tap${name} \
          ...
          vmInfo=$(sclctl vm show ${serviceName} ${name})
        ...
        '';
        serviceConfig.ExecStop = pkgs.writeShellScript "vm-${name}-stop" ''
          sclctl vm update ${serviceName} ${name} stopped
          sclctl vm delete ${serviceName} ${name}
        '';
