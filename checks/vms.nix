...
let
  ...
  imageRaw = runCommandNoCC "convert-RAW" { } ''
    ${qemu-utils}/bin/qemu-img convert -O raw ${image} $out
  '';
...
testers.nixosTest {
    ...
    virtualisation.vms = {
      ...
      virtualMachines = {
        testmachine = {
          ...
          cloudInitUserConfigFile = writeText "cloud-init-user.yaml" ''
            system_info:
              default_user:
                name: nixos
            password: nixos
          ...
          cloudInitNetworkConfigFile = writeText "cloud-init-network.yaml" ''
            version: 2
            ethernets:
              id0:
                match:
  ...
  testScript = ''
    ...
    with subtest("scl is accessible"):
      default.succeed("${scl}/bin/sclctl sc list")
